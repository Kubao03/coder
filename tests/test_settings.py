...
from settings import load_settings, update_settings, add_permission_rule, _merge_settings
...
class TestLoadSettings:
    ...
    def test_user_settings(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("settings.USER_SETTINGS_DIR", user_dir.parent / ".coder")
        # fix: point to the right dir
        monkeypatch.setattr("settings._user_settings_path", lambda: user_dir / "settings.json")
...
class TestUpdateSettings:
    def test_update_creates_file(self, tmp_path, monkeypatch):
        monkeypatch.setattr("settings._user_settings_path", lambda: tmp_path / ".coder" / "settings.json")
    ...
    def test_update_merges(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("settings._user_settings_path", lambda: settings_path)
